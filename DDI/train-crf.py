#!/usr/bin/env python3

import sys
from sklearn_crfsuite import CRF
# import pycrfsuite as CRF
import numpy as np
import argparse
from joblib import dump


def load_data(data):
    features = []
    labels = []
    for interaction in data:
        interaction = interaction.strip()
        interaction = interaction.split('\t')
        interaction_dict = {feat.split('=')[0]: feat.split('=')[1] for feat in interaction[1:]}
        features.append(interaction_dict)
        labels.append(interaction[0])
    return features, labels


if __name__ == '__main__':

    model_file = sys.argv[1]
    vectorizer_file = sys.argv[2]

    train_features, y_train = load_data(sys.stdin)
    y_train = np.asarray(y_train)

    crf = CRF(algorithm='lbfgs', max_iterations=100)
	
    # trainer=CRF.Trainer()

    X_train = [list(features.values()) for features in train_features]
    crf.fit(X_train,y_train)
    # print(len(X_train))
    # print(y_train.size)
    # for idx, item in enumerate(X_train):
    #     # print([item],[y_train[idx]])
    #     trainer.append([item],[y_train[idx]])
    
        
        
    # crf.fit(X_train, y_train)
    # trainer.train(model_file)
    # Save classifier (CRF model) - note that DictVectorizer is not needed for CRFs
    dump(crf, model_file)